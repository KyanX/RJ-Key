-- ═══════════════════════════════════════════════════════════════════
--  King Legacy Boss Detector v6.4
--  Area: Sea 2 | Executor: Delta Mobile (Android)
--
--  v6.4 dari v6.3:
--    CHEST NAMES:
--      - "Spawn Pit" ada spasi (fix dari SpawnPit)
--      - "Bottum" bukan "Buttom" (fix typo)
--      - Gem dihapus (false detection dari NPC chest)
--      - T3 tidak punya ChestSpawner (sesuai scan data)
--      - EyeRight/EyeLeft dikonfirmasi
--    TIER DETECTION:
--      - T3 = Dragon / Wing / "Spawn Pit"
--      - T1 = Top / Bottum (eksklusif T1, tidak ada di T2/T3)
--      - T2 = ChestTop/ChestBottum + SkullRetopo (tanpa T3/T1 parts)
--      - T4 = ChestTop/ChestBottum SAJA (tanpa marker lain, tentatif)
--    CHEST TP:
...
--      - Trigger autoStoreFruit() jika autoStore ON
--      - Atau tunggu 10 detik sebelum hop
--    ISLAND DETECTION:
...
--      - isPlayerPart: cek CHEST_SET dulu (O(1)), baru cek ancestry
...
-- ═══════════════════════════════════════════════════════════════════
--  CHEST DATA v6.4 (dikonfirmasi dari scan lapangan)
--
...
--      (tidak ada Dragon/Wing/Spawn Pit, tidak ada Top/Bottum)
--
--  T3: Dragon, Wing, "Spawn Pit", ChestTop, ChestBottum, EyeRight, EyeLeft
...
-- Tier 3 pakai nama dengan spasi — simpan terpisah untuk string match
local T3_SPAWN_PIT = "Spawn Pit"

-- Set untuk TP-prioritas (bukan reward biasa)
local TP_BLACKLIST = { Coins = true, MiniCoin = true }

-- Island anchors yang diketahui
local ISLAND_ANCHORS = { "HydraStand", "ClockTime" }  -- dikonfirmasi user
...
-- Optimisasi: jalan ke atas tree sampai ketemu model yang ada di charSet
...
local function detectTier(nameSet)
    -- T3: ada Dragon, Wing, atau "Spawn Pit"
    if nameSet["Dragon"] or nameSet["Wing"] or nameSet[T3_SPAWN_PIT] then
        return "T3 🐉"
    end
...
--  Cek CHEST_SET dan "Spawn Pit" (nama dengan spasi)
...
local function scanWorkspaceForChest(originPos)
    ...
    for _, obj in ipairs(workspace:GetDescendants()) do
        ...
        local nm = obj.Name
        local inSet = CHEST_SET[nm] or (nm == T3_SPAWN_PIT)
        ...
        if inSet and not isPlayerPart(obj) then
            ...
            if part then
                ...
                if d <= radius then
                    ...
                    elseif not fallbackPos and not TP_BLACKLIST[nm] then
...
-- ─── Find Store Button ────────────────────────────────────────────
-- Nama internal tombol di game: "Store"
-- Teks yang ditampilkan ke player: "Collect" (sesuai konfirmasi user)
--
-- Data dari debug v3:
--   Path: MainGui.StarterFrame.StatsFrame.PassivePage.Frame.PassiveBagFrame.Store
--   firesignal sudah return true tapi tidak menyimpan buah
--   → Penyebab: game handler cek Visible sebelum eksekusi
--   → Fix: force PassivePage + PassiveBagFrame + button sendiri = Visible=true
--          SEBELUM memanggil firesignal
--
-- Prioritas: PassiveBagFrame.Store > SlotInfoFrame.Store
local function findStoreButton()
    ...
    for _, obj in ipairs(pg:GetDescendants()) do
        if (obj:IsA("TextButton") or obj:IsA("ImageButton")) and obj.Name == "Store" then
            -- Force SEMUA ancestor tersembunyi agar visible (termasuk PassivePage, PassiveBagFrame)
            local p = obj.Parent
            while p and p ~= pg do
                if p:IsA("GuiObject") and not p.Visible then
                    pcall(function() p.Visible = true end)
                end
                p = p.Parent
            end
            -- Force tombol itu sendiri visible
            pcall(function() obj.Visible = true end)

            -- Scoring: prioritaskan PassiveBagFrame.Store di atas SlotInfoFrame.Store
            local parentName = obj.Parent and obj.Parent.Name or ""
            local isPrimary  = parentName == "PassiveBagFrame" and 4 or 0
            local chain = true
            p = obj.Parent
            while p and p ~= pg do
                if p:IsA("GuiObject") and not p.Visible then chain = false; break end
                p = p.Parent
            end
            local pos = obj.AbsolutePosition
            local onScreen = pos.X > 0 and pos.Y > 0
            local score = isPrimary + (chain and 2 or 0) + (onScreen and 1 or 0)
...
local function clickBtn(btn)
    ...
    -- firesignal: bypass visibility, paling reliable di executor
    ...
    task.wait(0.1)
    -- VIM di koordinat tombol (jika di layar)
    pcall(function()
...
-- ─── Auto Store Fruit ─────────────────────────────────────────────
--  Tombol yang dicari: "Collect" (di kanan bawah layar)
--  Buah di-equip SATU KALI saja di awal — tidak di-re-equip dalam loop
--  agar tidak terjadi ganti slot bolak-balik
function autoStoreFruit()
    setStatusGUI("🍎 Auto Collect Buah...", Color3.fromRGB(255,180,80), 2)
    local bp = lp:FindFirstChild("Backpack")
    if not bp then
        setStatusGUI("❌ Backpack tidak ada", Color3.fromRGB(255,80,80), 4)
        return false
    end
    local tools = {}
    ...
    -- Slot 4 = buah (dikonfirmasi user)
    local fruit = tools[4]
    if not fruit then
        for i = #tools, 1, -1 do
            if not isSwordTool(tools[i]) then fruit = tools[i]; break end
        end
    end
    if not fruit then
        setStatusGUI("❌ Buah tidak ada (tools=" .. #tools .. ")", Color3.fromRGB(255,80,80), 4)
        return false
    end
    ...
    -- Equip buah SEKALI SAJA di sini — tidak di-equip lagi di dalam loop
    local hum = getHumanoid()
    if hum then
        pcall(function() hum:EquipTool(fruit) end)
        task.wait(1.5)  -- tunggu UI passive storage muncul
    end

    -- Cari tombol "Store" (nama internal) → force ancestor visible → klik
    -- Tombol di-force visible agar firesignal benar-benar dieksekusi oleh game
    -- (game handler mungkin punya cek Visible sebelum simpan buah)
    for attempt = 1, 10 do
        local btn = findStoreButton()  -- mencari Name=="Store", force ancestor visible di dalam
        if btn then
            -- Tunggu satu frame agar AbsolutePosition terupdate setelah force visible
            task.wait(0.08)
            setStatusGUI("🍎 Store button! (" .. attempt .. ")", Color3.fromRGB(255,220,80), 2)
            clickBtn(btn)
            task.wait(1)
            setStatusGUI("✅ Buah disimpan: " .. fruit.Name, Color3.fromRGB(80,255,150), 5)
            return true
        end
        -- Belum ditemukan — buah mungkin belum di-equip sempurna, atau UI delay
        setStatusGUI("🍎 Tunggu UI Store... (" .. attempt .. "/10)", Color3.fromRGB(255,160,60), 2)
        task.wait(0.8)
    end

    setStatusGUI("❌ Store button tidak ditemukan\nJalankan store_debug_v3.lua", Color3.fromRGB(255,80,80), 6)
    return false
end
...
local function startFight(boss)
    ...
    task.spawn(function()
        ...
        -- Equip loop paralel (setiap 0.3s)
        task.spawn(function()
        ...
        if bossHRP then
            local hrp = getHRP()
            if hrp then hrp.CFrame = bossHRP.CFrame + Vector3.new(5, 2, 0) end
        end
        ...
        local timerZ = 0
        local timerX = 1.5
        setStatusGUI("⚔️ Fighting boss!", Color3.fromRGB(255,120,60), 2)

        while isFighting and isRunning do
            local dt = task.wait(0.1)  -- 0.1s = lebih cepat untuk serangan

            -- Update posisi boss (boss bisa bergerak)
            if bossHRP and bossHRP.Parent then
                savedBossPos = bossHRP.Position
            end

            -- Cek boss masih ada di workspace (hilang = mati)
            local bossHum = boss:FindFirstChildOfClass("Humanoid")
            if not bossHum or bossHum.Health <= 0 or not boss.Parent then
                isFighting  = false
                isPostFight = true
                setStatusGUI("✅ Boss mati!", Color3.fromRGB(100,255,150), 3)

                task.wait(2)  -- tunggu chest mulai spawn

                -- Cari dan TP ke chest (scan 1500m dari posisi boss)
                local motorDetected = false
            ...
            if bossHRP and bossHRP.Parent then
                local hrp = getHRP()
                if hrp and (hrp.Position - bossHRP.Position).Magnitude > 10 then
                    hrp.CFrame = bossHRP.CFrame + Vector3.new(5, 2, 0)
                end
            end

            -- Serangan (klik kiri, lebih cepat dari v6.3)
            ...
            -- Skill X (setiap 3 detik, offset 1.5s)
            timerX = timerX + dt
            if timerX >= 3 then
                timerX = 0
                pcall(function()
...
ttlLbl.Text                   = "👾  BOSS DETECTOR v6.4"
...
local btnStoreNow   = Btn("🍎 Test Collect Buah",        Color3.fromRGB(80,48,14))
...
Sec("── INFO TIER ──")
Lbl(
    "T1: ChestSpawner+Top/Bottum/SkullRetopo/Eye\n"..
    "T2: ChestSpawner+ChestTop/ChestBottum/SkullRetopo\n"..
    "T3: Dragon/Wing/Spawn Pit+ChestTop/ChestBottum\n"..
...
updateToggles()
isRunning = true
setStatusGUI("▶ Running v6.4...", Color3.fromRGB(100,220,150))
...
print("[BossDetector v6.4] Script loaded. Panel: tombol 👾 kiri atas.")
